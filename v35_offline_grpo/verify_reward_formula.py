...
def _solution(mode: str, totals: dict[str, int], reasoning: str = "") -> str:
    ...
    perception = {phase: _phase_row(phase, totals[phase]) for phase in reward.PHASES}
    ...
    return (
        ...
        f"<current_v>{json.dumps(totals)}</current_v>"
        "<signal>ETWT</signal>"
    )
...
def main() -> None:
    ...
    expected_invalid = -reward.P_MAX * (1.0 - math.exp(-300 / reward.TAU))
    assert math.isclose(invalid_score, expected_invalid), (invalid_score, expected_invalid)
    assert invalid_result["format_reward"] == 0.0
    print("PASS: format gate and R = traffic + 0.5 * perception - reasoning_penalty.")
